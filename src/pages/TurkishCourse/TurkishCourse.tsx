...
import { allWords, alphabet, dialogs, grammarTopics, wordCategories } from './turkishData';
import { auth } from '../../firebase';
import { recordStudentCourseProgress } from '../../utils/studentAccount';
import styles from './TurkishCourse.module.css';
...
const TurkishCourse: React.FC = () => {
  ...
  const [quizOptions, setQuizOptions] = useState<string[]>(() => shuffle(allWords).slice(0, 4).map((word) => word.tr));
  const [selectedAnswer, setSelectedAnswer] = useState('');
  const [score, setScore] = useState(() => Number(localStorage.getItem('turkish_score') || 0));
  const [learned, setLearned] = useState<string[]>(() => {
    try {
      return JSON.parse(localStorage.getItem('turkish_learned_words') || '[]');
    } catch {
      return [];
    }
  });
  ...
  const activeWords = useMemo(() => {
    ...
    return source.filter((word) => {
      const text = `${word.ky} ${word.tr} ${word.reading} ${word.example}`.toLowerCase();
  ...
  const toggleLearned = (word: string) => {
    const next = learned.includes(word)
      ...
      : [...learned, word];
    setLearned(next);
    localStorage.setItem('turkish_learned_words', JSON.stringify(next));
  };
  ...
  const completeLessonWord = () => {
    if (!learned.includes(lessonWord.tr)) {
      toggleLearned(lessonWord.tr);
    }
  ...
  const newQuestion = () => {
    ...
    const wrongOptions = shuffle(allWords.filter((word) => word.tr !== nextWord.tr))
      .slice(0, 3)
      .map((word) => word.tr);
    setQuizWord(nextWord);
    setQuizOptions(shuffle([nextWord.tr, ...wrongOptions]));
    setSelectedAnswer('');
  };
  ...
  const checkAnswer = (answer: string) => {
    ...
    if (answer === quizWord.tr) {
      const nextScore = score + 1;
      setScore(nextScore);
      localStorage.setItem('turkish_score', String(nextScore));
    }
  };
      ...
      <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className={styles.hero}>
        <div className={styles.heroText}>
          <div className={styles.heroCopy}>
            ...
            <h1>Түрк тилин сабак-сабак болуп үйрөн</h1>
            <p>
          ...
          <div className={styles.heroShowcase}>
            <span>Günün cümlesi</span>
            <strong>Türkçe öğreniyorum.</strong>
            <small>Мен түркчө үйрөнүп жатам.</small>
            <button type="button" onClick={() => speak('Türkçe öğreniyorum')}>
              <Volume2 size={17} />
              Угуу
            </button>
          </div>
          ...
          <section className={styles.lessonHub}>
            ...
            <div className={styles.pathGrid}>
              {wordCategories.map((category, index) => {
                const learnedCount = category.words.filter((word) => learned.includes(word.tr)).length;
        ...
        <section className={styles.lessonStage}>
              ...
              <div className={styles.vocabularyStage}>
                ...
                <div className={styles.duoLesson}>
                  ...
                  <div className={styles.lessonCard}>
                    ...
                    <button type="button" className={styles.bigSoundBtn} onClick={() => speak(lessonWord.tr)}>
                      <Volume2 size={30} />
                    </button>
                    <h3>{lessonWord.tr}</h3>
              ...
              <div className={styles.wordsPanel}>
                <div className={styles.wordsToolbar}>
                  ...
                  <label className={styles.searchBox}>
                    ...
                    <input
                      ...
                      placeholder="Кыргызча же түркчө издөө..."
                    />
                  </label>
                </div>
                ...
                <div className={styles.wordsGrid}>
                  {activeWords.map((word) => {
                    const isLearned = learned.includes(word.tr);
                    return (
                      <article key={`${word.tr}-${word.ky}`} className={isLearned ? styles.learnedCard : styles.wordCard}>
                        <div className={styles.wordTopline}>
                          <small>{word.reading}</small>
                          {isLearned && <em>OK</em>}
                        </div>
                        <button type="button" className={styles.soundBtn} onClick={() => speak(word.tr)} aria-label="Угуу">
                          <Volume2 size={18} />
                        </button>
                        <h3>{word.tr}</h3>
                        <p>{word.ky}</p>
                        <span>{word.example}</span>
                        <button type="button" className={styles.learnBtn} onClick={() => toggleLearned(word.tr)}>
          ...
          {activeSection === 'quiz' && (
            <div className={styles.quizCard}>
              ...
              <h2>{quizWord.ky}</h2>
              <p>Бул сөз түркчө кандай болот?</p>
              <div className={styles.options}>
                {quizOptions.map((option) => {
                  const isCorrect = selectedAnswer && option === quizWord.tr;
                  const isWrong = selectedAnswer === option && option !== quizWord.tr;
                  return (
                    <button
                      key={option}
                      type="button"
              ...
              {selectedAnswer && (
                <div className={styles.quizResult}>
                  <strong>{selectedAnswer === quizWord.tr ? 'Туура!' : `Туура жооп: ${quizWord.tr}`}</strong>
