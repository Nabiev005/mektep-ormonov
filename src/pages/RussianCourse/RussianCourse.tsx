...
import { allWords, alphabet, dialogs, grammarTopics, wordCategories } from './russianData';
...
const RussianCourse: React.FC = () => {
  ...
  const [quizOptions, setQuizOptions] = useState<string[]>(() => shuffle(allWords).slice(0, 4).map((word) => word.ru));
  const [selectedAnswer, setSelectedAnswer] = useState('');
  const [score, setScore] = useState(() => Number(localStorage.getItem('russian_score') || 0));
  const [learned, setLearned] = useState<string[]>(() => {
    try {
      return JSON.parse(localStorage.getItem('russian_learned_words') || '[]');
    } catch {
      return [];
    }
  });
  ...
  const activeWords = useMemo(() => {
    ...
    return source.filter((word) => {
      const text = `${word.ky} ${word.ru} ${word.reading} ${word.example}`.toLowerCase();
  ...
  const toggleLearned = (word: string) => {
    const next = learned.includes(word)
      ...
      : [...learned, word];
    setLearned(next);
    localStorage.setItem('russian_learned_words', JSON.stringify(next));
  };
  ...
  const completeLessonWord = () => {
    if (!learned.includes(lessonWord.ru)) {
      toggleLearned(lessonWord.ru);
    }
  ...
  const newQuestion = () => {
    ...
    const wrongOptions = shuffle(allWords.filter((word) => word.ru !== nextWord.ru))
      .slice(0, 3)
      .map((word) => word.ru);
    setQuizWord(nextWord);
    setQuizOptions(shuffle([nextWord.ru, ...wrongOptions]));
    setSelectedAnswer('');
  };
  ...
  const checkAnswer = (answer: string) => {
    ...
    if (answer === quizWord.ru) {
      ...
      setScore(nextScore);
      localStorage.setItem('russian_score', String(nextScore));
    }
  };
      ...
      <motion.section initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className={styles.hero}>
        <div className={styles.heroText}>
          <div className={styles.heroCopy}>
            ...
            <h1>Орус тилин сабак-сабак болуп үйрөн</h1>
            <p>
          ...
          <div className={styles.heroShowcase}>
            ...
            <strong>Я учу русский язык.</strong>
            <small>Мен орус тилин үйрөнүп жатам.</small>
            <button type="button" onClick={() => speak('Я учу русский язык')}>
              <Volume2 size={17} />
              Угуу
            </button>
          </div>
          ...
          <section className={styles.lessonHub}>
            ...
            <div className={styles.pathGrid}>
              {wordCategories.map((category, index) => {
                const learnedCount = category.words.filter((word) => learned.includes(word.ru)).length;
        ...
        <section className={styles.lessonStage}>
              ...
              <div className={styles.vocabularyStage}>
                ...
                <div className={styles.duoLesson}>
                  ...
                  <div className={styles.lessonCard}>
                    ...
                    <button type="button" className={styles.bigSoundBtn} onClick={() => speak(lessonWord.ru)}>
                      <Volume2 size={30} />
                    </button>
                    <h3>{lessonWord.ru}</h3>
              ...
              <div className={styles.wordsPanel}>
                <div className={styles.wordsToolbar}>
                  ...
                  <label className={styles.searchBox}>
                    ...
                    <input
                      ...
                      placeholder="Кыргызча же орусча издөө..."
                    />
                  </label>
                </div>
                ...
                <div className={styles.wordsGrid}>
                  {activeWords.map((word) => {
                    const isLearned = learned.includes(word.ru);
                    return (
                      <article key={`${word.ru}-${word.ky}`} className={isLearned ? styles.learnedCard : styles.wordCard}>
                        <div className={styles.wordTopline}>
                          <small>{word.reading}</small>
                          {isLearned && <em>OK</em>}
                        </div>
                        <button type="button" className={styles.soundBtn} onClick={() => speak(word.ru)} aria-label="Угуу">
                          <Volume2 size={18} />
                        </button>
                        <h3>{word.ru}</h3>
                        <p>{word.ky}</p>
                        <span>{word.example}</span>
                        <button type="button" className={styles.learnBtn} onClick={() => toggleLearned(word.ru)}>
          ...
          {activeSection === 'quiz' && (
            <div className={styles.quizCard}>
              ...
              <h2>{quizWord.ky}</h2>
              <p>Бул сөз орусча кандай болот?</p>
              <div className={styles.options}>
                {quizOptions.map((option) => {
                  const isCorrect = selectedAnswer && option === quizWord.ru;
                  const isWrong = selectedAnswer === option && option !== quizWord.ru;
                  return (
                    <button
                      key={option}
                      type="button"
              ...
              {selectedAnswer && (
                <div className={styles.quizResult}>
                  <strong>{selectedAnswer === quizWord.ru ? 'Туура!' : `Туура жооп: ${quizWord.ru}`}</strong>
